...
from graph2vector2 import MPNN_DDI, get_data
from data_prepare2 import CustomData
# from graph2verctor1 import MVN_DDI, get_data
...
class ProjectNetwork(nn.Module):
    ...
    def forward(self, node_feat, edge_feat):
        ...
        s_feat = node_feat[:, :-1, :].contiguous().view(-1,128)
        q_feat = node_feat[:, -1, :].repeat(1, num_data-1, 1).contiguous().view(-1,128)

        dis = torch.exp(-self.distance(s_feat,q_feat)).view(-1,10,1)
...
class RelationNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer2 = nn.Sequential(
                        ...
                        nn.ReLU(),
                        nn.MaxPool1d(5))
        self.fc0 = nn.Sequential(nn.Linear(22, 128), self.reluDrop, nn.BatchNorm1d(128))
        ...
        self.fc1 = nn.Sequential(nn.Linear(448,256), self.reluDrop, nn.BatchNorm1d(256))  # for 1722 input dimension
...
def get_bipartite_graph(mol_graph_1,mol_graph_2):
    x1 = np.arange(0,mol_graph_1.x.size(0))
    x2 = np.arange(0,mol_graph_2.x.size(0))
    edge_list = torch.LongTensor(np.meshgrid(x1,x2))
    edge_list = torch.stack([edge_list[0].reshape(-1),edge_list[1].reshape(-1)])
    return edge_list


def get_line_graph_edge_index(smol_graph_data):
    line_graph_edge_index = torch.LongTensor([])
    if smol_graph_data.edge_index.T.nelement() != 0:
        conn1 = (smol_graph_data.edge_index.T[:, 1].unsqueeze(1) == smol_graph_data.edge_index.T[:, 0].unsqueeze(
            0)) & (
                        smol_graph_data.edge_index.T[:, 0].unsqueeze(1) != smol_graph_data.edge_index.T[:,
                                                                            1].unsqueeze(0))
        line_graph_edge_index = conn1.nonzero(as_tuple=False).T
...
def main():
    ...
    sys.stdout = Logger(str(excludeLabel)+'-db86-dgnn-rare-10way.txt')
    ...
    with open('./drugbank_data.pkl', 'rb') as f:
        drug_graph = pickle.load(f)


    # init neural network
    print("init neural networks")
    model_nn = MPNN_DDI(70,64,3)
    relation_network = RelationNetwork()
    co_relation = Corelation()
    project_network = ProjectNetwork(128,128,2)
    ...
    for episode in range(EPISODE):
        ...
        for i in range(Support_NUM_PER_CLASS*NUM_WAYS):
            ...
            # smol1_graph_data.line_graph_edge_index = get_line_graph_edge_index(smol1_graph_data)
            # smol1_graph_data.line_graph_edge_index = get_line_graph_edge_index(smol2_graph_data)

            # sb_graph = BipartiteData(get_bipartite_graph(smol1_graph_data, smol2_graph_data), smol1_graph_data.x, smol2_graph_data.x)
            support_h_list.append(smol1_graph_data)
            support_t_list.append(smol2_graph_data)
            # support_b_list.append(sb_graph)
        ...
        for i in range(QUERY_NUM_PER_CLASS*NUM_WAYS):
            ...
            # qmol1_graph_data.line_graph_edge_index = get_line_graph_edge_index(qmol1_graph_data)
            # qmol2_graph_data.line_graph_edge_index = get_line_graph_edge_index(qmol2_graph_data)

            # qb_graph = BipartiteData(get_bipartite_graph(qmol1_graph_data, qmol2_graph_data), qmol1_graph_data.x,qmol2_graph_data.x)
            query_h_list.append(qmol1_graph_data)
            query_t_list.append(qmol2_graph_data)
            # query_b_list.append(qb_graph)
        ...
        mag_support_feature = model_nn(support_loader)
        mag_query_feature = model_nn(query_loader)
        # ******** graph2vector ***********
        ...
        coattr = adj.view(-1,NUM_WAYS+1)

        list_coattr_tensor = []
        for i in range(0,QUERY_NUM_PER_CLASS*NUM_WAYS*(NUM_WAYS+1),NUM_WAYS+1):
            # 5 shot
            ...
            # 10 shot
            list_coattr_tensor.append(torch.cat((coattr[i + 0].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 1].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 2].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 3].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 4].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 5].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 6].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 7].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 8].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
            list_coattr_tensor.append(torch.cat((coattr[i + 9].view(1,-1), coattr[i + 10].view(1,-1)), dim=1))
        ...
        if (episode + 1) % 5000 == 0: #1000
            ...
            for i in range(TEST_EPISODE):
                ...
                for i in range(Support_NUM_PER_CLASS * NUM_WAYS):
                    ...
                    # smol1_graph_data.line_graph_edge_index = get_line_graph_edge_index(smol1_graph_data)
                    # smol2_graph_data.line_graph_edge_index = get_line_graph_edge_index(smol2_graph_data)
                ...
                list_coattr_tensor = []
                for i in range(0, NUM_WAYS*Support_NUM_PER_CLASS*(NUM_WAYS+1), NUM_WAYS+1):
                    # 5 shot
                    # list_coattr_tensor.append(torch.cat((coattr[i + 0].view(1, -1), coattr[i + 5].view(1, -1)), dim=1))
                    # list_coattr_tensor.append(torch.cat((coattr[i + 1].view(1, -1), coattr[i + 5].view(1, -1)), dim=1))
                    # list_coattr_tensor.append(torch.cat((coattr[i + 2].view(1, -1), coattr[i + 5].view(1, -1)), dim=1))
