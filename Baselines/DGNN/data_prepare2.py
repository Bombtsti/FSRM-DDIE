...
if __name__ == '__main__':
    ...
    drug_id_mol_tup = []
    symbols = list()
    conn=sqlite3.connect("../METADDIEdata/drugbank_drug.db")
    smileFile = pd.read_sql("select * from drug",conn)
    for i in range(1706):
        smiles[smileFile.loc[i][0]] = (smileFile.loc[i][1])
        # drug_id_mol_graph_tup = [(smileFile.loc[i][0], Chem.MolFromSmiles(smileFile.loc[i][3].strip()))]
        # mols[smileFile.loc[i][0]] = generate_drug_data(Chem.MolFromSmiles(smileFile.loc[i][3].strip()))

    for id, smiles in smiles.items():
        mol = Chem.MolFromSmiles(smiles.strip())
        if mol is not None:
            drug_id_mol_tup.append((id, mol))
            symbols.extend(atom.GetSymbol() for atom in mol.GetAtoms())
    ...
    symbols = list(set(symbols))
    drug_data = {id: generate_drug_data(mol, symbols) for id, mol in tqdm(drug_id_mol_tup, desc='Processing drugs')}
    save_data(drug_data, '/drugbank_data.pkl')
