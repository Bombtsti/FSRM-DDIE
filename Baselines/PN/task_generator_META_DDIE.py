...
conn = sqlite3.connect("../METADDIEdata/drugbank_few_shot_DDIE.db")
cur=conn.cursor()
...
class MetaDDIETask(object):
    def __init__(self,all_event,num_ways,support_num,query_num,state,excludeLabel):
        ...
        if state=="train":
            select_class = random.sample(range(1,64),num_ways)
            #select_class = [random.randint(1,all_event) for _ in range(num_ways)]
        if state=='test':
            select_class = random.sample(range(76,87),num_ways) #64 76 86
        ...
        self.support_labels = [labels[int(x[4])] for x in self.support_samples] # d1: 3 d2:4
        self.query_labels = [labels[int(x[4])] for x in self.query_samples]
