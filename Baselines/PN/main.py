...
#         self.fc1 = nn.Sequential(nn.Linear(224,128), self.reluDrop, nn.BatchNorm1d(128))  # for 1722 input dimension
...
class RelationNetwork(nn.Module):
    ...
    def forward(self,x):
        ...
        z = self.encoder(x)
        z = z.view(z.size(0), -1)
        z_proto = z[:10].view(10, 1, -1).mean(1)
        zq = z[10:]
...
def main():
    ...
    sys.stdout = Logger(str(excludeLabel)+'-db86-pn-rare-10way.txt')
